...
then	
	...
	/usr/bin/docker run --rm=true --hostname="${DOCKER_NAME}" --name="${DOCKER_NAME}" -p 8080:8080 -p 2019:2019 -e ETCDCTL_PEERS=${ETCDCTL_PEERS} ${DOCKER_IMG} /tmp/node-provisioning.sh node-provisioning-${MY_IP} ${MY_IP}
else 
