...
then	
	...
	/usr/bin/docker run --rm=true --hostname="${DOCKER_NAME}" --name="${DOCKER_NAME}" -p 8090:8080 -p 2020:2019 -v /opt/inaetics/bundles:/bundles -e HOSTPORT=8090 -e ETCDCTL_PEERS=${ETCDCTL_PEERS} ${DOCKER_IMG} /tmp/node-provisioning.sh node-provisioning-${MY_IP} ${MY_IP}
else 
